# portscout 设计文档 — 本机端口服务监听与注册工具

> 2026-07-16 · 状态：待批准 · 名称暂定 portscout（可改）

## 背景与目标

多个 AI agent（Claude Code / Cursor / Antigravity）在同一台机器上并行开发，各自启动 dev server，导致：端口冲突启动失败、vite 静默换端口后 agent 找不到自己的服务、会话结束后遗留孤儿进程占着端口没人认领（实测本机 8901 就挂着一个父进程已退出的 http.server）。

目标：一条命令回答「哪个端口被谁占、属于哪个项目」；agent 启动服务前可原子预留端口；孤儿进程可发现、可清理。

**设计原则：扫描发现是基座（零侵入，覆盖所有不配合的 agent），注册预留是增强（给配合的 agent 用）。**
...
portscout whois <port> [--json]     # 单端口详情：项目目录/命令/来源/注册信息
portscout claim <name> [--prefer N] [--range A-B] [--json]
                                    # 原子预留空闲端口，stdout 仅输出端口号
                                    # 幂等：同 (项目,name) 重复 claim 返回原端口
                                    # 粘性：历史分配过的端口优先复用，地址可预测
portscout release <name>            # 仅释放预留记录，不停止进程；服务仍在监听时告警提示
portscout stop <port|name> [--force|--gui] [--json]
                                    # 带护栏地停止端口上的服务（见下节安全规则）
                                    # --gui：护栏改用 osascript 确认框+系统通知（菜单栏用）
portscout gc [--kill-orphans]       # 清理死注册记录；列出（可选杀掉）孤儿服务
portscout watch                     # 终端仪表盘，实时刷新
portscout menubar [--install]       # 输出 SwiftBar/xbar 插件协议文本；--install 写入插件目录
```

- `list` 默认过滤噪音（IDE 内部 language server、系统服务），`--all` 显示全部。
- `claim` 的典型用法：`PORT=$(portscout claim mu-frontend --prefer 8000)`，冲突时自动顺延并返回实际端口；stdout 纯数字保证可脚本化，人类信息走 stderr。
...
- 并发安全：写入前以 `mkdir ~/.portscout/.lock`（O_EXCL 语义）取锁，超时 2s 强夺（附 pid 检查）。
...
- `gc --kill-orphans` 复用同一终止逻辑，且只作用于孤儿级
- agent 约定中可放心写「端口被占用 `portscout stop` 处置」——误杀由工具拦截，而非依赖 agent 自觉

## watch 仪表盘

零依赖 ANSI 终端渲染（不引入 ink/blessed）：2 秒间隔重扫重绘，表格同 `list`，差异高亮——新增行绿色、消失行红色保留一轮、孤儿行黄色。`q` 退出。适合调试时盯看；被动余光监视用下面的菜单栏形态。

## 菜单栏（SwiftBar/xbar 插件）

`portscout menubar` 按 SwiftBar/xbar 插件协议输出文本，宿主 app 定时执行渲染（推荐 SwiftBar，`brew install swiftbar`，开源免费；刷新间隔由插件文件名约定，如 `portscout.5s.sh`）。

菜单结构：

```
⚓5 ⚠1                                ← 常驻标题：服务数 + 异常数（有孤儿/漂移变警示色）
---
⚠ 8901 孤儿服务 · site-platform      ← 顶层一行一个服务
...
- 点击动作全部挂 CLI 命令（`bash=portscout param1=stop param2=8000 terminal=false refresh=true`），菜单栏只是渲染器，逻辑仍集中在 CLI。
- **护栏的 GUI 形态（stop --gui）**：孤儿/自己的服务直接停 + 系统通知结果；他人活跃服务先弹 osascript 确认对话框（列明归属），确认即 force；失败以系统通知告知。护栏规则与终端模式完全一致，仅呈现方式不同，实现零新增依赖（osascript 系统自带）。
- 安装：`portscout menubar --install` 自动写入 SwiftBar 插件目录一个 5 秒间隔的包装脚本。
...
- 仅支持 macOS（lsof/ps 参数按 Darwin 写），Linux 留接口后补
- 结构：`src/scan.ts`（归属引擎）、`src/registry.ts`（注册表+锁）、`src/commands/*.ts`（六命令）、`src/render.ts`（表格/ANSI）、`bin/portscout`
